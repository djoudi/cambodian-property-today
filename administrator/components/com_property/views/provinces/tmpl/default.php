...
?>
<form action="<?php echo JRoute::_('index.php?option=com_property&view=provinces'); ?>" method="post" name="adminForm" id="adminForm">
    ...
    <table class="adminlist">
        ...
        <?php foreach ($this->items as $i => $item) :
            ...
            $urlEdit        = "index.php?option=com_property&task=province.edit&id={$item->id}" ;
            $canCreate	= $user->authorise('core.create',		'com_property.province.'.$item->id);
            $canEdit	= $user->authorise('core.edit',			'com_property.province.'.$item->id);
            $canCheckin	= $user->authorise('core.manage',		'com_checkin') || $item->checked_out==$user->get('id') || $item->checked_out==0;
            $canChange	= $user->authorise('core.edit.state',	'com_property.province.'.$item->id) && $canCheckin;
        ?>
